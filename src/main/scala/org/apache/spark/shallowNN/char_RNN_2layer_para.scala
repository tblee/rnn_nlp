...
object char_RNN_2layer_para {
  ...
  extends Serializable {
    ...
    def step(input_seq: Array[Int],
      ...
      (loss, clip(dWxh1), clip(dWhh1), clip(dWhy1), clip(dby1), clip(dbh1),
        clip(dWxh2), clip(dWhh2), clip(dWhy2), clip(dby2), clip(dbh2), 1)
    }
    ...
    def fit() = {
      ...
      while (epoch >= 0) {
        val gradient_seq = train2id.map(window => step(window,
          Array(DenseVector.zeros[Double](hidden_dim), DenseVector.zeros[Double](hidden_dim))))
        val gradients = gradient_seq.reduce{
          case (x, y) =>
            (x._1 + y._1, x._2 + y._2, x._3 + y._3, x._4 + y._4, x._5 + y._5, x._6 + y._6, x._7 + y._7,
              x._8 + y._8, x._9 + y._9, x._10 + y._10, x._11 + y._11, x._12 + y._12)
        }

        val (loss, dWxh1, dWhh1, dWhy1, dby1, dbh1, dWxh2, dWhh2, dWhy2, dby2, dbh2, count) = gradients

        // update parameters
        val ct = count.toDouble
        update_param(dWxh1 / ct, dWhh1 / ct, dWhy1 / ct, dby1 / ct, dbh1 / ct, dWxh2 / ct, dWhh2 / ct, dWhy2 / ct, dby2 / ct, dbh2 / ct)
